...
import com.didispace.excel2.ExportExcel;
...
public class Excel2Controller {
  ...
  @GetMapping(value = "/exportProduct2")
  public String exportProduct() {
    ...
    for (int i = 2; i < number; i = i + 2) {
      if (i < 2 * fialList.size() + 2) {
        ...
        row3Cell.setCellValue(new HSSFRichTextString(fialList.get(m).toString()));
        m++;
      } else {
        row3Cell = row3.createCell(i);
        row3Cell.setCellStyle(cellStyle);
        row3Cell.setCellValue(new HSSFRichTextString(errorList.get(n).toString()));
        n++;
      }
    }
    ...
    sheet.addMergedRegion(new Region(2, (short) start, 2, (short) (number - 1)));
    ...
    for (int i = 5; i < number; i++) {
      ...
      for (int j = 0; j <= number; j++) {
        exportExcel.cteateCell(
            wb, row, (short) j, HSSFCellStyle.ALIGN_CENTER_SELECTION, String.valueOf(j));
      }
    }
    ...
    exportExcel.outputExcel("./didispace-excel/workbook2.xls");
